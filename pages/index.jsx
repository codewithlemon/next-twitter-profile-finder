...
const HomePage = () => {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const response =
        selectedMethod === "id"
          ? await fetch(`/api/profile/by-id/${userIdentifier.current.value}`)
          : await fetch(
              `/api/profile/by-username/${userIdentifier.current.value}`
            )
  ...
  return (
    <main className="max-w-xl w-full mx-auto px-4 py-3">
      {/* Search box */}
      <form
